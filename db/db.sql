...
CREATE TABLE `sensor_last` (
  `type` enum('temperature','humidity','pressure','voltage','wind-speed','wind-direction','sky-temperature','ccd-exposure','ccd-average','ccd-gain','ccd-bin','stars-count','ai-clear','ai-cloud','docker-started') NOT NULL,
  `channel` int(10) unsigned NOT NULL,
  `date` int(10) unsigned NOT NULL,
  `val` double NOT NULL,
