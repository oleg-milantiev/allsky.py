...
class qhyccd():
    def __init__(self):
        # create sdk handle
        self.sdk= CDLL('/camera/'+ os.getenv('ARCH') +'/libqhyccd.so')
        ...
        self.mode = 0 # 0 = single frame; 1 = stream mode
...
#        max = c_double()
#        step = c_double()
#        self.sdk.GetQHYCCDParamMinMaxStep(self.cam, CONTROL_ID.CONTROL_GAIN, byref(min), byref(max), byref(step))
##        self.sdk.GetQHYCCDParamMinMaxStep(self.cam, CONTROL_ID.CONTROL_USBTRAFFIC, byref(min), byref(max), byref(step))
#        print(min)
#        print(max)
#        print(step)
    ...
    """ Set camera gain 0..100 """
    def SetGain(self, gain):
        self.sdk.SetQHYCCDParam(self.cam, CONTROL_ID.CONTROL_GAIN, c_double(gain))
        print("Set gain to", 
                self.sdk.GetQHYCCDParam(self.cam, CONTROL_ID.CONTROL_GAIN))
