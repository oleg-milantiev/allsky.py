...
hot = []
hot.append(Image.open('/opt/allsky.py/camera/common/hot-1.png'))
hot.append(Image.open('/opt/allsky.py/camera/common/hot-2.png'))
...
for f in os.listdir('./'):
	if f[-3:] == 'fit':
		...
		print(f +': bin'+ str(hdu.header['XBINNING']))

		hotCount = 0
		for y in range(1, hdu.data.shape[0] - 1):
			for x in range(1, hdu.data.shape[1] - 1):
				if hot[hdu.header['XBINNING'] - 1].getpixel((x, y)) == 255:
					hotCount += 1
